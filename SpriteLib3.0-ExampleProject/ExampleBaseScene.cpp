...
void ExampleBaseScene::InitScene(float windowWidth, float windowHeight)
{
	...
	//Big rock
	{
		...
		ECS::GetComponent<Transform>(entity).SetPosition(0.f, 0.f, 10.f);
	...
	//small rock
	{
		...
		std::string filename = "Short_rock.png";
		...
		b2BodyDef tempDef;
		tempDef.type = b2_dynamicBody;
		tempDef.position.Set(float32(-127.f), float32(50.0f));
		...
		unsigned int bitHolder = EntityIdentifier::WaterBit()| EntityIdentifier::GroundBit() | EntityIdentifier::SpriteBit() | EntityIdentifier::TransformBit() | EntityIdentifier::PhysicsBit();
		ECS::SetUpIdentifier(entity, bitHolder, "ground");
	}
